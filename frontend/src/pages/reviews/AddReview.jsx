...
const AddReview = () => {
    ...
    const validate = () => {
        ...
        if (!url.trim()) errs.url = 'Ссылка обязательна'; else if (!/^https?:\/\/.+/.test(url)) errs.url = 'Введите корректный URL';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('/reviews/create-review', {
                title: title, description: description, url: url, rating: rating, content: content
            });
            navigate('/');
        } catch (error) {
